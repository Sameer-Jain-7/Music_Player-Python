...
class View:
    def __init__(self, top=None):
        ...
        self.songName.configure(text='''Add Songs to PLay''')
        ...
        self.playButton.place(relx=0.568, rely=0.266, height=40, width=40)
        ...
        self.stopButton.place(relx=0.713, rely=0.266, height=40, width=40)
        ...
        self.label.place(relx=0.02, rely=0.0, height=204, width=204)
        ...
        self._img20 = tk.PhotoImage(file="./icons/music_photo.png")
        self.label.configure(image=self._img20)

        self.playList = ScrolledListBox(top)
        self.playList.place(relx=0.025, rely=0.40, relheight=0.493, relwidth=0.950)

        self.playList.configure(background="#E0EBEE")
        self.playList.configure(disabledforeground="#a3a3a3")
        self.playList.configure(font="Helvetica")
        ...
        self.playList.configure(width=15)

        self.previousButton = tk.Button(top)
        self.previousButton.place(relx=0.495, rely=0.266, height=40, width=40)
        ...
        self.nextButton.place(relx=0.64, rely=0.266, height=40, width=40)
        ...
        self._img11 = tk.PhotoImage(file="./icons/next.png")
        self.nextButton.configure(image=self._img11)
        self.nextButton.configure(text='''Label''')
        ...
        self.vol_scale = ttk.Scale(top,cursor="hand2")
        ...
        self._img8 = tk.PhotoImage(file="./icons/fovorite.png")
    ...
    def setup_player(self):
        try:
            ...
            if self.my_player.get_db_status():
                messagebox.showinfo("Success!","Welcome to Melodyo!!")
            else:
    ...
    def play_song(self):
        self.x = self.sel_song_index_tuple
        self.sel_song_index_tuple=self.playList.curselection()
        while True:
            self.i = self.i+1
            if self.i<2:
                break
            if self.x!=self.sel_song_index_tuple:
                self.stop_song()
                self.playButton.configure(image=self._img1)
                break
            else:
                self.playButton.configure(image=self._img1)
                break
    ...
    def closewindow(self):
        ...
        if answer is True:
            self.my_player.stop_song()
            messagebox.showinfo("Have a good day","Thank you for using Melodyo..!!")
    ...
    def next_song(self):
        try:
            ...
            if len(self.sel_song_index_tuple) == 0:
                raise NoSongSelectedError("No Song is being Played")
    ...
    def previous_song(self):
        try:
            ...
            if self.prev_song_index==-1:
                ...
                if self.prev_song_index==0:
                    raise NoSongSelectedError("Only one song present")
            self.playList.select_clear(0,tk.END)
    ...
    def load_songs_from_favorites(self):
        try:
          ...
          if result[0]=="No songs present in favourites!":
    ...
    def remove_song_from_favourites(self):
        ...
        try:
            ...
            if result=="Song deleted from your favourites":
                ...
                self.my_player.stop_song()
                self.songName.configure(text='''Select Song to PLay''')
                self.songTimePassed.configure(text="")
                self.songTotalDuration.configure(text="")
                self.songProgress.stop()
                messagebox.showinfo("Success!!!", result)
            else:
                messagebox.showinfo("Not Present!!!", result)
        except(NoSongSelectedError) as ex1:
            messagebox.showerror("Error!", ex1)
        except(DatabaseError) as ex2:
            messagebox.showerror("DB Error!", "Song can not be removed!!!")
